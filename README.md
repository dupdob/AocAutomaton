...
site. Also, it **automatically deals with response throttling and submit the answer** as soon as possible. It will also
avoid submitting known wrong answers, such as null, zero or negatives values, previously tried answers or answers that
are outside the known range (when you got 'answer is too low' and/or 'too high' results).
---- 
## Philosophy
...
The `ReuseExample` attribute is used here to reuse the example inpout from the first part of the puzzle. It takes the example
id (here 1) and the expected result (here 50346) as parameters. In this case, this is equivalent to `[Example("100756", 50346)]`.
